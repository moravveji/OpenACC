...
## Optimizing Loop(s)
The three loops in `set_xy()` function is much more efficient with `kernels` directive than with `parallel loop`. So, one must basically play around with different parallel regions on what is the best. In the `gen_gauss2d()` function, and for `nx=10001, ny=10001`, each iteration of the workflow took the following amounts of time with the given fine-tuned parallelism options:
